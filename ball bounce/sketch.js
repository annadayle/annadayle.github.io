  ...
  for (let i=0; i<ballArray.length; i++) {
    for(let j=0; j<ballArray.length; j++) {
      ...
      if (i != j) {
        ...
        ballArray[i].checkColliding(ballArray[j]);
      }
    }
    ballArray[i].move();
    ballArray[i].display();
...
class Ball {
  ...
  checkColliding(otherBall) {
    let sumOfRadii = this.radius + otherBall.radius;
    let distanceCenter = dist(this.x, this.y, otherBall.x, otherBall.y);
    if (sumOfRadii > distanceCenter) {
      // this.someColor = "red";
